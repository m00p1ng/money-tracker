...
export function dateOnly(value: string | Date): string {
  return toDateOnly(value instanceof Date
    ? value
    : new Date(value))
}
...
export function getPresetRange(preset: DateRangePreset, now = new Date()): DateRange {
  ...
  if (preset === 'last-7d') {
    return { start: toDateOnly(addDays(now, -6)), end: toDateOnly(now) }
  }
  if (preset === 'last-30d') {
    return { start: toDateOnly(addDays(now, -29)), end: toDateOnly(now) }
  }
  if (preset === 'last-90d') {
