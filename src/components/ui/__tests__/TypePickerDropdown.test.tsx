...
import { TypePickerDropdown } from '../TypePickerDropdown'
...
describe('TypePickerDropdown', () => {
  ...
  it('renders income label when value is income', () => {
    render(<TypePickerDropdown value="income" onChange={vi.fn()} />)
  ...
  it('calls onChange and closes dropdown when a type row is clicked', async () => {
    ...
    render(<TypePickerDropdown value="expense" onChange={onChange} />)
    await userEvent.click(screen.getByRole('button', { name: /expense/i }))
  ...
  it('closes dropdown on outside mousedown', async () => {
    render(
      <div>
        <TypePickerDropdown value="expense" onChange={vi.fn()} />
